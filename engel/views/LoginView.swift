...
// MARK: - Two-Factor Verification View
...
struct TwoFAView: View {
        ...
        VStack(spacing: 0) {
            ScrollView(showsIndicators: false) {
                VStack(spacing: 0) {
                    ...
                    // Hero globe
                    GlobeView(tone: .green, size: 72, entryCount: 6, pulse: true)
                        .frame(width: 72, height: 72)

                    Text("Enter your code")
                        .font(AppTypography.display(size: 28, weight: .light))
                        .foregroundStyle(ThemeTokens.colors.ink)
                        .padding(.top, 24)

                    if let phone = session.phoneNumber {
                        Text("Sent to \(phone)")
                            .font(AppTypography.mono(size: 12, weight: .regular))
                            .foregroundStyle(ThemeTokens.colors.inkDim)
                            .padding(.top, 6)
    ...
    private var codeFieldSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            TextField("000000", text: $code)
                .font(AppTypography.mono(size: 24, weight: .medium))
                .foregroundStyle(ThemeTokens.colors.ink)
    ...
    private var verifyButton: some View {
        Button {
            ...
            .background(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .fill(isCodeValid && !session.isVerifying
                          ? ThemeTokens.colors.ink
                          : ThemeTokens.colors.ink.opacity(0.25))
            )
        }
        .buttonStyle(.plain)
        .disabled(!isCodeValid || session.isVerifying)
    }
}
